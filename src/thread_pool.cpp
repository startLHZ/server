...
void* thread_pool::worker(void* arg) {
    ...
    while (1)
    {
        ...
        locker.unlock();
        char *arg[3];
        arg[0] = (char*) curTask.fd;
        arg[1] = (char*) pool;
        arg[2] = 0;
        curTask.function(arg);
...
void taskFunc(char* arg[])
{
    int curfd = (int) *arg[0];
    thread_pool* p = (thread_pool*) arg[1];
    if(curfd == p->lfd)
    {
        // 建立新的连接
        ...
        p->ev.events = EPOLLIN | EPOLLET;    // 读缓冲区是否有数据
        p->ev.data.fd = cfd;
        p->ret = epoll_ctl(p->epfd, EPOLL_CTL_ADD, cfd, &p->ev);
        if(p->ret == -1)
        {
        ...
        while(1)
        {
            ...
            if(len == 0)
            {
                ...
                printf("客户端断开了连接...\n");
                // 将这个文件描述符从epoll模型中删除
                epoll_ctl(p->epfd, EPOLL_CTL_DEL, curfd, NULL);
                close(curfd);
                break;
            }
            else if(len > 0)
